...
static lv_color_t battery_image_buffer[ZMK_SPLIT_BLE_CENTRAL_PERIPHERALS + SOURCE_OFFSET][5 * 8];
...
static void set_battery_symbol(lv_obj_t *widget, struct battery_state state) {
    if (state.source >= ZMK_SPLIT_BLE_CENTRAL_PERIPHERALS + SOURCE_OFFSET) {
        return;
    }
...
int zmk_widget_dongle_battery_status_init(struct zmk_widget_dongle_battery_status *widget, lv_obj_t *parent) {
    ...
    for (int i = 0; i < ZMK_SPLIT_BLE_CENTRAL_PERIPHERALS + SOURCE_OFFSET; i++) {
